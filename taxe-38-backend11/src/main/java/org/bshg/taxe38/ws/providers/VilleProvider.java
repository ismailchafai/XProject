...
import org.bshg.taxe38.ws.converter.VilleConverter;
...
public ResponseEntity<List<VilleDto>> findAllOptimized() {
var result = service.findAllOptimized();
var resultDto = converter.toDto(result);
return ResponseEntity.ok(resultDto);
}
@GetMapping("/paginated")
public ResponseEntity<Pagination<VilleDto>> findPaginated(
...
@PostMapping("/all")
public ResponseEntity<List<VilleDto>> save(@RequestBody List<VilleDto> dtos) {
...
@PutMapping("/all")
public ResponseEntity<List<VilleDto>> update(@RequestBody List<VilleDto> dtos) {
if (dtos == null) return ResponseEntity.status(HttpStatus.CONFLICT).build();
var item = converter.toItem(dtos);
var result = service.update(item);
var resultDto = converter.toDto(result);
...
@DeleteMapping
public ResponseEntity<VilleDto> delete(@RequestBody VilleDto dto) {
if (dto == null) return ResponseEntity.status(HttpStatus.CONFLICT).build();
var item = converter.toItem(dto);
service.delete(item);
...
@DeleteMapping("/all")
public ResponseEntity<List<VilleDto>> delete(@RequestBody List<VilleDto> dtos) {
if (dtos == null) return ResponseEntity.status(HttpStatus.CONFLICT).build();
var item = converter.toItem(dtos);
service.delete(item);
